...
parser.add_argument("-p", "--LABELBOX_PROJECT_NAME", help="Enter your project name here : ")
parser.add_argument("-d", "--LABELBOX_DATASET_NAME", help="Enter your dataset name here : ")
parser.add_argument(
    "-i",
    "--INPUT_BBDF_DIR",
...
def upload_annotations(project: object, data_row: object, labelbox_data: object) -> None:
...
if __name__ == "__main__":
    ...
    project = next(client.get_projects(where=Project.name == args.LABELBOX_PROJECT_NAME), None)
    dataset = next(client.get_datasets(where=Dataset.name == args.LABELBOX_DATASET_NAME), None)
    ...
    for data_row in tqdm(data_rows):
        ...
        try:
            ...
            labelbox_data = bbdf.to_labelbox_data(data_row, schema_lookup, KEYFRAME_WINDOW)
