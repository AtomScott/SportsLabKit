    ...
    for player in player_ids: # cycle through players individually
        ...
        if smoothing:
            if filter_=='Savitzky-Golay':
                ...
                vx.loc[:] = signal.savgol_filter(vx.loc[:],window_length=window,polyorder=polyorder)
                vy.loc[:] = signal.savgol_filter(vy.loc[:],window_length=window,polyorder=polyorder)        
                ...
                # calculate first half velocity
                vx.loc[:] = np.convolve( vx.loc[:] , ma_window, mode='same' ) 
                vy.loc[:] = np.convolve( vy.loc[:] , ma_window, mode='same' )      
                # calculate second half velocity
                vx.loc[:] = np.convolve( vx.loc[:] , ma_window, mode='same' ) 
                vy.loc[:] = np.convolve( vy.loc[:] , ma_window, mode='same' ) 
