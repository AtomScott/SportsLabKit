...
def generate_pitch_control_for_event(
    ...
    ball_start_pos = np.round(
        np.array(
            [
                tracking_home.loc[frame_id, "ball_x"],
                tracking_home.loc[frame_id, "ball_y"],
            ]
        ),
        2,
    )
    ...
    attacking_players = initialise_players(
        tracking_home.loc[frame_id], "Home", params, GK_numbers[0]
    )
    defending_players = initialise_players(
        tracking_away.loc[frame_id], "Away", params, GK_numbers[1]
    )
