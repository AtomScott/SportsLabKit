...
def generate_pitch_control_for_event(frame_id, tracking_home, tracking_away, params, GK_numbers, field_dimen = (105.,68.,), n_grid_cells_x = 50, offsides=True):
    ...
    ball_start_pos = np.round(np.array([tracking_home.loc[frame_id, 'ball_x'], tracking_home.loc[frame_id, 'ball_y']]), 2)
    ...
    # if pass_team=='Home':
    attacking_players = initialise_players(tracking_home.loc[frame_id],'Home',params,GK_numbers[0])
    defending_players = initialise_players(tracking_away.loc[frame_id],'Away',params,GK_numbers[1])
