...
class Model:
    def __init__(
        ...
        self.state_length = self.dim_pos * (self.order_pos + 1) + self.dim_size * (
            self.order_size + 1
        )
    ...
    def _calc_idxs(self):
        ...
        pos_idxs = [pidx * (self.order_pos + 1) for pidx in range(self.dim_pos)]
    ...
    def build_Q(self):
        ...
        q_pos = (
            var_pos
            if self.order_pos == 0
            else Q_discrete_white_noise(dim=self.order_pos + 1, dt=self.dt, var=var_pos)
        )
